...
struct vertex
{
	...
	*/
	vertex(int id=0, string c="", string s="", string cou="", latlon ll = latlon())
	{
		ID = id;
		city = c;
		state = s;
...
public:
	...
	*     string state
	*/
	vertex *createVertex(string city, string state, string county, latlon ll)
	{
		return new vertex(id++, city, state, county,ll);
	}
	...
	*     void
	*/
	int addVertex(string city, string state, string county, double lat = 0.0, double lon = 0.0)
	{
		...
		vertex *temp = createVertex(city, state, county,latlon(lat, lon));
...
graph loadGraphCSV(string filename, int max = 0)
{
	...
	for (CSVIterator loop(file); loop != CSVIterator(); ++loop)
	{
		...
		if (cityCheck.find(city) == cityCheck.end())
		{
			...
			if (state != "PR") {
				G.addVertex(city, state, county,lat, lon);
				i++;
			}
		}
...
/**
* loadPRGraphCSV - loads a graph with the given csv with cities from puerto rico
* Params:
*     string filename  - filename to open
* Returns
*     graph
